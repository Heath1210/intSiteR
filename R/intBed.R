...
intBed <- function(mgBed,
  ...
  if(nrow(bed)==0){
    ...
    if(mixBarcode){
      putative_barc = as.character(filter(barcode_table,Freq/sum(Freq) > 0.05)$Var1) %>% as.numeric()
    }else{
      putative_barc = as.character(arrange(barcode_table,desc(Freq))$Var1[1]) %>% as.numeric()
    }
